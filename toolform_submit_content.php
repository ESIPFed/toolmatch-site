...
try {
	if(isset($_POST['submit'])) {
		...
		$tool_servers = array();
		$tool_servers = $_POST['toolserver'];
		
		$tool_protocols = array();
		$tool_protocols = $_POST['toolprotocol'];
  ...
  $rdf_start = '<toolmatch:Tool rdf:about="&toolmatchi;' . $toolname . '">
    ...
    <doap:release>
      <doap:Version rdf:about="&toolmatchi;' . $toolname . '_' . $toolvers . '">
		...
		//rdf for tool input formats (optional)	
		$rdf_tool_inputs = '';
    ...
    <toolmatch:hasInputFormat rdf:resource="' . $input . '"/>';
		...
		//rdf for tool output formats (optional)	
		$rdf_tool_outputs = '';
    ...
    <toolmatch:hasOutputFormat rdf:resource="' . $output . '"/>';
		...
		//rdf for tool servers(optional)	
		$rdf_tool_servers = '';
		if (!empty($tool_servers)) {
			foreach ($tool_servers as $server) {
				$rdf_tool_servers = $rdf_tool_servers . '
    <toolmatch:canUseDataServer rdf:resource="' . $server . '"/>';
			}
		}
		
		//rdf for tool protocols(optional)	
		$rdf_tool_protocols = '';
		if (!empty($tool_protocols)) {
			foreach ($tool_protocols as $protocol) {
				$rdf_tool_protocols = $rdf_tool_protocols . '
    <toolmatch:canUseAccessProtocol rdf:resource="' . $protocol . '"/>';
			}
		}
		
		//rdf for tool capability (optional)	
		$rdf_vistypes = '';
		if (!empty($vistypes)) {
    ...
    <toolmatch:isOfType rdf:resource="' . $type . '"/>';
		...
		$rdf = $rdf_beg . $rdf_start . $rdf_tool_inputs . $rdf_tool_outputs . $rdf_tool_servers . $rdf_tool_protocols . $rdf_vistypes . $rdf_tooltypes . $rdf_end;
